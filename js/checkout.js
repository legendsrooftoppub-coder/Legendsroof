document.addEventListener('DOMContentLoaded', function(){
  ...
  function setCart(c){ try { localStorage.setItem('legendsCart', JSON.stringify(c)); } catch(_){} }
  ...
  function computeDistanceFromCoords(lat, lng){
    ...
    distanceService.getDistanceMatrix({
      ...
      if (status !== 'OK') { toast('Could not compute distance','error'); return; }
      ...
      if (row && row.distance && row.status === 'OK') {
        ...
        toast('Distance unavailable for selected address','error');
      }
    });
  }
  ...
  form.addEventListener('submit', async function(e){
    ...
    // reCAPTCHA v2 validation (enforce only if widget and API are present)
    ...
    if (recaptchaWidget && typeof grecaptcha !== 'undefined' && !recaptchaToken) {
    ...
    // Include reCAPTCHA token (may be empty in non-enforced environments)
